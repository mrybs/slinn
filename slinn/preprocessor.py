...
class Preprocessor:
	def __init__(self, open_quote: str='<%', close_quote: str='%>'):
		self.open_quote = open_quote
		self.close_quote = close_quote
		self.pattern = lambda self: fr'{self.open_quote}\s*{i_dat}\s*{self.close_quote}'

	def replace(self, text: str, data: dict) -> str:
		for i, dat in enumerate(data):
			i_dat = list(dat)[0]
			text = re.sub(self.pattern(), dat[i_dat], text)
		return text
